...
def main(local_terraform_user_config_path: str, backend_terraform_dir: str):
    ...
    """
    tuc = load_terraform_user_config(config_path=local_terraform_user_config_path)

    environment = Environment(loader=FileSystemLoader(TEMPLATES_DIR))
    template = environment.get_template("backend_tfvars.txt")
    content = template.render(
        profile=tuc.aws_profiles.backend.profile,
        region=tuc.aws_profiles.backend.region,
        bucket_name=tuc.backend.bucket_name,
        table_name=tuc.backend.dynamodb_table_name,
    )
...
if __name__ == "__main__":
    ...
    main(args.local_terraform_user_config_path, args.backend_terraform_dir)